...
export const pickCurrentWeatherData = (data) => {
  return {
    ...
    temperature: {
      metric: `${data[0].Temperature.Metric.Value.toFixed()}°${
        data[0].Temperature.Metric.Unit
      }`,
      imperial: `${data[0].Temperature.Imperial.Value.toFixed()}°${
    ...
    wind: {
      metric: `${data[0].Wind.Speed.Metric.Value.toFixed()} ${
        data[0].Wind.Speed.Metric.Unit
      }`,
      imperial: `${data[0].Wind.Speed.Imperial.Value.toFixed()} ${
        data[0].Wind.Speed.Imperial.Unit
      }`,
    },
    windGust: {
      metric: `${data[0].WindGust.Speed.Metric.Value.toFixed()} ${
        data[0].WindGust.Speed.Metric.Unit
      }`,
      imperial: `${data[0].WindGust.Speed.Imperial.Value.toFixed()} ${
        data[0].WindGust.Speed.Imperial.Unit
      }`,
    },
    humidity: `${data[0].RelativeHumidity}%`,
    indoorHumidity: `${data[0].IndoorRelativeHumidity}%`,
    dewPoint: {
      metric: `${data[0].DewPoint.Metric.Value.toFixed()}°${
        data[0].DewPoint.Metric.Unit
      }`,
      imperial: `${data[0].DewPoint.Imperial.Value.toFixed()}°${
        data[0].DewPoint.Imperial.Unit
      }`,
    },
    pressure: {
      metric: `${data[0].Pressure.Metric.Value.toFixed()} ${
        data[0].Pressure.Metric.Unit
      }`,
      imperial: `${data[0].Pressure.Imperial.Value.toFixed()} ${
        data[0].Pressure.Imperial.Unit
      }`,
    },
    cloudCover: `${data[0].CloudCover}%`,
    visibility: {
      metric: `${data[0].Visibility.Metric.Value.toFixed()} ${
        data[0].Visibility.Metric.Unit
      }`,
      imperial: `${data[0].Visibility.Imperial.Value.toFixed()} ${
        data[0].Visibility.Imperial.Unit
      }`,
    },
    cloudCeiling: {
      metric: `${data[0].Ceiling.Metric.Value.toFixed()} ${
        data[0].Ceiling.Metric.Unit
      }`,
      imperial: `${data[0].Ceiling.Imperial.Value.toFixed()} ${
        data[0].Ceiling.Imperial.Unit
      }`,
    },
  };
};
...
export const pickOneDayForecastData = (data) => {
  ...
  const maxUVIndex = dailyForecast.AirAndPollen.find(
    (p) => p.Name === "UVIndex"
  );
  const findParams = (name) =>
    dailyForecast.AirAndPollen.find((param) => param.Name === name).Category;
  const allergy = {
    grass: {
  ...
  return {
    ...
    dayForecast: {
      ...
      temperature: `${dailyForecast.Temperature.Maximum.Value.toFixed()}°${
      ...
      wind: `${
        dailyForecast.Day.Wind.Direction.Localized
      } ${dailyForecast.Day.Wind.Speed.Value.toFixed()} ${
        dailyForecast.Day.Wind.Speed.Unit
      }`,
      windGust: `${
        dailyForecast.Day.WindGust.Direction.Localized
      } ${dailyForecast.Day.WindGust.Speed.Value.toFixed()} ${
        dailyForecast.Day.WindGust.Speed.Unit
      }`,
    ...
    nightForecast: {
      ...
      temperature: `${dailyForecast.Temperature.Minimum.Value.toFixed()}°${
      ...
      wind: `${
        dailyForecast.Night.Wind.Direction.Localized
      } ${dailyForecast.Night.Wind.Speed.Value.toFixed()} ${
        dailyForecast.Night.Wind.Speed.Unit
      }`,
      windGust: `${
        dailyForecast.Night.WindGust.Direction.Localized
      } ${dailyForecast.Night.WindGust.Speed.Value.toFixed()} ${
        dailyForecast.Night.WindGust.Speed.Unit
      }`,
...
export const pickTwelveHourlyData = (data) => {
  const res = data.map((d) => ({
    ...
    wind: `${d.Wind.Direction.Localized} ${d.Wind.Speed.Value.toFixed()} ${
      d.Wind.Speed.Unit
    }`,
...
export const pickFiveDayData = (data) => {
  const res = data.DailyForecasts.map((d) => ({
    ...
    dayForecast: {
      ...
      maxUVIndex: `${d.AirAndPollen.find((p) => p.Name === "UVIndex").Value} ${
        d.AirAndPollen.find((p) => p.Name === "UVIndex").Category
      }`,
      ...
      wind: `${
        d.Day.Wind.Direction.Localized
      } ${d.Day.Wind.Speed.Value.toFixed()} ${d.Day.Wind.Speed.Unit}`,
      windGust: `${
        d.Day.WindGust.Direction.Localized
      } ${d.Day.WindGust.Speed.Value.toFixed()} ${d.Day.WindGust.Speed.Unit}`,
    ...
    nightForecast: {
      ...
      wind: `${
        d.Night.Wind.Direction.Localized
      } ${d.Night.Wind.Speed.Value.toFixed()} ${d.Night.Wind.Speed.Unit}`,
      windGust: `${
        d.Night.WindGust.Direction.Localized
      } ${d.Night.WindGust.Speed.Value.toFixed()} ${
        d.Night.WindGust.Speed.Unit
      }`,
...
export const pickTopCitiesData = (data) => {
  return data.map((d) => ({
    ...
    temperature: {
      metric: `${d.Temperature.Metric.Value.toFixed()}°${
        d.Temperature.Metric.Unit
      }`,
      imperial: `${d.Temperature.Imperial.Value.toFixed()}°${
        d.Temperature.Imperial.Unit
      }`,
    },
